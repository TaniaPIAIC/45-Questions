...
const current_users:string[]=["Ria","Tia","Mia","Lia","Ayman"];
const new_users:string[]=["Ayamn","Urooba","Ayan","Ria"];
for(const new_user of new_users){
  const isUsernameTaken=current_users.some((current_user)=>current_user.toLowerCase()===new_user.toLowerCase());  

if(isUsernameTaken){
console.log(`The user name ${new_user},is not available please enter a new user`);
